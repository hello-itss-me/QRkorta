...
interface PositionCardProps {
  ...
  onClonePosition: (positionId: string) => void;
  ...
  onReturnGroupToUnallocated: (positionId: string, itemIds: string[]) => void;
...
const PositionCard: React.FC<PositionCardProps> = ({
  ...
  const handleDragStartGrouped = (groupedItem: GroupedRepairItem) => {
    onDragStart(groupedItem, position.id);
  };

  const handleReturnGroupToUnallocated = (positionGroup: PositionGroup) => {
    const itemIdsToReturn = positionGroup.items.flatMap(groupedItem => groupedItem.groupedIds);
    
    if (itemIdsToReturn.length > 0) {
      onReturnGroupToUnallocated(position.id, itemIdsToReturn);
    }
  };
        ...
        <div className="flex items-center space-x-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            {isEditing ? (
              ...
              <div className="flex items-center space-x-2">
                <h3 
                  className="text-lg font-semibold text-gray-900 truncate cursor-pointer hover:bg-gray-100 px-2 py-1 rounded transition-colors"
                ...
                <button
                  onClick={() => onAddNewWorkGroup(position.id)}
                  className="p-1 text-blue-600 hover:bg-blue-100 rounded-full transition-colors flex-shrink-0"
                  title="Добавить новую группу работ"
                >
                  <PlusCircle className="w-5 h-5" />
                </button>
                <button
                  onClick={() => onClonePosition(position.id)}
                  className="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
                  title="Клонировать позицию"
                >
        ...
        {position.items.length === 0 && !isDragOver ? (
          ...
          <div className="space-y-4">
            {groupedItemsByWorkType.workTypes.map((workTypeGroup) => {
              ...
              const workTypeIncomeTotal = allIncomeItems.reduce((sum, item) => sum + item.totalRevenue, 0);
              const workTypeExpenseTotal = allExpenseItems.reduce((sum, item) => sum + Math.abs(item.totalRevenue), 0);
                ...
                <div key={workTypeGroup.workType} className="border border-gray-200 rounded-lg overflow-hidden">
                  <div className="w-full px-3 py-2 bg-blue-100 hover:bg-blue-200 flex items-center justify-between transition-colors">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        onClick={() => onAddNewItemToWorkGroup(position.id, workTypeGroup.workType)}
                        className="p-1 text-blue-600 hover:bg-blue-200 rounded-full transition-colors"
                        title="Добавить новую позицию в группу"
                      >
                  ...
                  {!workTypeGroup.isCollapsed && (
                    <div className="bg-white">
                      {workTypeGroup.positions.map((positionGroup) => {
                        ...
                        const positionIncomeTotal = incomeItems.reduce((sum, item) => sum + item.totalRevenue, 0);
                        const positionExpenseTotal = expenseItems.reduce((sum, item) => sum + Math.abs(item.totalRevenue), 0);
                                      ...
                                      <div className="pl-16 pr-2 py-2 space-y-2">
                                        {incomeItems.map((groupedItem) => (
                                          <GroupedRepairItemCard
                                            ...
                                            onReplaceItem={onReplaceItem ? (item) => onReplaceItem(position.id, item) : undefined}
                                          />
                                        ))}
                                      </div>
                                    )}
